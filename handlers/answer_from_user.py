...
def register_answer_form_handler(dp: Dispatcher):
    ...
    async def start_answer(call: CallbackQuery, callback_data, state: FSMContext):
        ...
        if select_is_active_meeting(call.from_user['id'], user_about[0]):
            ...
            await call.message.answer(
                ...
                'опишите Ваши впечатления о МЕСТЕ в котором Вы встречались, отправьте это сообщение прямо в чат👇\n'
                'Можно добавить одно фото и текст')
            user_id = call.from_user['id']
            await state.update_data(from_user=user_id, user_about=user_about, photo=[])
        else:
            await call.message.edit_text('Ваша встреча уже завершена ранее.')
    ...
    async def answer_place(message: types.Message, state: FSMContext):
        ...
        if 'photo' in message:
            ...
            text = message.text
        if text:
            await message.answer('Отлично! Теперь опишите собеседника и отправьте сообщение в этот чат👇\n'
            'Можно добавить одно фото и текст')
    ...
    async def next_answer(message: types.Message, state: FSMContext):
        ...
        if text:
            ...
            await message.answer('👍Прекрасно! Спасибо! Когда Вы попадёте в новую встречу, сюда придет приглашение.')
            await state.finish()
            add_rating(message.from_user["id"])
            bd_make_free_user(message.from_user["id"])

            create_answer_xlsx(data['from_user'], data['user_about'][0], data['txt_about_place'], data['txt_about_people'])
